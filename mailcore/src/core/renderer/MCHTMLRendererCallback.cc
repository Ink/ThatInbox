...
{
    return MCSTR("<div style='background-color:#eee; padding:10px'>\
                 {{#HASSUBJECT}}\
                 <H2>{{EXTRACTEDSUBJECT}}</H2>\
                 {{/HASSUBJECT}}\
                 {{#NOSUBJECT}}\
                 <H2><i>No Subject</i></H2>\
                 {{/NOSUBJECT}}\
                 {{#HASFROM}}\
                 <table>\
                 <tr><td style='width: 100px'><b>From:</b></td><td>{{FROM}}</td></tr>\
                 {{/HASFROM}}\
                 {{#HASTO}}\
                 <tr><td><b>To:</b></td><td>{{TO}}</td></tr>\
                 {{/HASTO}}\
                 {{#HASCC}}\
                 <tr><td><b>Cc:</b></td><td>{{CC}}</td></tr>\
                 {{/HASCC}}\
                 {{#HASBCC}}\
                 <tr><td><b>Bcc:</b></td><td>{{BCC}}</td></tr>\
                 {{/HASBCC}}\
                 {{#NORECIPIENT}}\
                 <tr><td><b>To:</b></td><td><i>Undisclosed recipient</i></td></tr>\
                 {{/NORECIPIENT}}\
                 <tr><td><b>Date:</b></td><td>{{LONGDATE}}</td></tr>\
                 </table>\
                 </div>");
}
...
{
    //return MCSTR("<div>{{HEADER}}</div><div style='margin: 100px; padding:100px'>{{BODY}}</div>");
    return MCSTR("<div style=\"padding-bottom: 20px;\">{{HEADER}}</div><div>{{BODY}}</div>");
}
